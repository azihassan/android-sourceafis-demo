...
class ForeignTemplate {
	...
	ForeignTemplate(FingerprintTemplate... templates) {
		...
		dimensions = StreamSupport.stream(fingerprints).findFirst().map(f -> f.dimensions).orElse(null);
	}
	...
	private int measure() {
		...
		int total = fixed + StreamSupport.stream(fingerprints).mapToInt(f -> f.measure(format)).sum();
		if (format == ForeignFormat.ANSI_378_2004) {
